...
let newUser = TryCatch(async (req, res, next) => {
...
let login = TryCatch(
    async (req, res, next) => {
        ...
        if (!user) return next(new ErrorHandler("Invalid Username Or Invalid Password", 404));

        const isMatch = await compare(password, user.password);

        if (!isMatch) {
            return next(new ErrorHandler("Invalid Username Or Invalid Password", 404));
        }

        sendToken(res, user, 200, "Signin successfully")

    }
)

let getMyProfile = TryCatch(async (req, res, next) => {
