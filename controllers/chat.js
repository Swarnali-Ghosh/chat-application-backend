...
const getMyChats = TryCatch(async (req, res, next) => {
    ...
    if (chats.length == 0 || chats == null || chats == [] || chats == undefined) {
...
const addMembers = TryCatch(async (req, res, next) => {
    ...
    if (chat.creator.toString() !== req.user.toString())
        return next(new ErrorHandler("You are not allowrd to add members", 403));
    ...
    const allUsersName = allNewMembers.map((i) => i.name).join(",");  // get all users name

    emitEvent(
        req,
        ALERT,
        chat.members, `${allUsersName} have been added to ${chat.name} group`
    );
...
const removeMember = TryCatch(async (req, res, next) => {
    ...
    if (!chat.groupChat)
        return next(new ErrorHandler("This is not a group chat", 400));
    // only creator can remove group members
    if (chat.creator.toString() !== req.user.toString())
    ...
    // remove users from this member array
...
const leaveGroup = TryCatch(async (req, res, next) => {
    ...
    //   creator want to leave the group then random group member will be creator
    if (chat.creator.toString() === req.user.toString()) {
...
const sendAttachments = TryCatch(async (req, res, next) => {
    ...
    //Data object for save to database
    const messageForDB = {
        content: "",
        attachments,
        sender: me._id,
